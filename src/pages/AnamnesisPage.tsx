...
const COMPLAINT_CATEGORIES = {
  "Общие": ["Общая слабость", "Потеря веса", "Повышение температуры", "Потливость", "Снижение аппетита", "Нарушение сна", "Отеки", "Утомляемость", "Озноб", "Жажда"],
  "Дыхательная система": ["Кашель", "Одышка", "Кровохарканье", "Боли в грудной клетке", "Осиплость голоса", "Чувство нехватки воздуха", "Свистящее дыхание", "Ночной кашель", "Выделение мокроты"],
  "Пищеварительная система": ["Тошнота", "Рвота", "Нарушение глотания (дисфагия)", "Боли в животе", "Нарушение стула (запор/диарея)", "Примесь крови в стуле", "Изжога", "Вздутие живота", "Снижение аппетита", "Горечь во рту", "Желтуха"],
  "Мочеполовая система": ["Нарушение мочеиспускания", "Примесь крови в моче", "Боли в пояснице", "Выделения из половых путей", "Учащенное мочеиспускание", "Боли при мочеиспускании", "Недержание мочи", "Никтурия"],
  "Кожа и мягкие ткани": ["Наличие опухолевидного образования", "Изменение цвета кожи", "Зуд", "Изъязвление", "Увеличение лимфоузлов", "Появление родинок", "Сыпь", "Незаживающие раны", "Выпадение волос"],
  "Неврология": ["Головная боль", "Головокружение", "Нарушение чувствительности", "Слабость в конечностях", "Судороги", "Нарушение зрения", "Нарушение речи", "Нарушение памяти", "Шум в ушах", "Потеря сознания"],
  "Костно-мышечная система": ["Боли в суставах", "Боли в мышцах", "Боли в костях", "Боли в позвоночнике", "Ограничение подвижности", "Отёк суставов", "Деформация суставов", "Утренняя скованность", "Хруст в суставах", "Слабость мышц", "Судороги в мышцах", "Патологические переломы"]
};
...
export function AnamnesisPage() {
  ...
  const { register, watch, setValue, getValues, reset } = useForm({
    defaultValues: data.anamnesis || { complaints: [], harmful_factors: [], smoking: false, alcohol: false }
  });
  const selectedComplaints = watch("complaints") || [];
  const selectedHarmfulFactors = watch("harmful_factors") || [];
  const smoking = watch("smoking") || false;
  const alcohol = watch("alcohol") || false;

  useEffect(() => {
    reset(data.anamnesis || { complaints: [], harmful_factors: [] });
  }, [consultationId, data.anamnesis, reset]);
  ...
  const toggleHarmfulFactor = (factor: string) => {
    const current = getValues("harmful_factors") || [];
    let newFactors;
    if (current.includes(factor)) {
      newFactors = current.filter((f: string) => f !== factor);
    } else {
      newFactors = [...current, factor];
    }
    setValue("harmful_factors", newFactors);
    handleSave();
  };

  const toggleSmoking = () => {
    const current = !getValues("smoking");
    setValue("smoking", current);
    handleSave();
  };

  const toggleAlcohol = () => {
    const current = !getValues("alcohol");
    setValue("alcohol", current);
    handleSave();
  };
              ...
              <div className="flex flex-wrap gap-2">
                {HARMFUL_FACTORS.map((factor) => (
                  <button
                    key={factor}
                    type="button"
                    onClick={() => toggleHarmfulFactor(factor)}
                    className={cn(
                      "px-3 py-1.5 rounded-full text-sm font-medium border transition-colors",
                      selectedHarmfulFactors.includes(factor)
